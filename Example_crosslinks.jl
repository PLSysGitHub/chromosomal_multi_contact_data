...
The example data has been generated with code from https://github.com/harjuj/Crosslinked_chromosomes
...
gr(label="",size=(540,500),xticks=(0:100:400, 0:1000:4000), yticks=(100:100:400,1000:1000:4000), #units in kb
        foreground_color_legend = nothing, background_color_legend=nothing)

#Change names of files and directories to analyse different data

num_samples_dict=load("./Extract_triplets/Crosslinks/sample_counts.jld2", "num_samples")
files=keys(num_samples_dict) #file names for different crosslinker potentials

for f in files
        contact_file="Contact_files/crosslinks/$f.txt"

        out_dir="./Output/Crosslinks_examples/$f/"
        if !isdir(out_dir) mkdir(out_dir) end

        #Read in the data
        P=readdlm(contact_file)
        P0_file="./Contact_files/P0_crosslinks.txt"
        P0=readdlm(P0_file)
        heatmap(half_half(P,P0), color=cgrad(:dense), clims=(0,0.03))
        png(out_dir*"hic")

        num_samp=num_samples_dict[f]
        triplet_file="Triplet_files/Crosslinks/$f.h5"
        ...
        plot(xs,P_3_s, label=["Data" "Ideal pol." "Loop-extr." "Pairwise int."],
